...
import {
  ...
  Type, AlignLeft, CheckSquare, Image, Loader2, ListChecks,
  BarChart3,
  Wallet, LinkIcon, Unlink
} from 'lucide-react';
import { useAuthStore } from '@/lib/store';
import {
  logTemplateApi, tradeAccountApi,
  type FieldType, type TradeAccount,
      ...
      setFields(
        (t.fields || [])
          ...
          .map(f => ({
            ...
            scorecard: f.fieldType === 'SCORECARD' || f.fieldType === 'SCORED_CHOICE'
              ? {
                  weight: (f.fieldType === 'SCORECARD' && (f.scorecard?.weight === undefined || f.scorecard?.weight === null)) ? '' : (f.fieldType === 'SCORECARD' ? String(f.scorecard?.weight) : ''),
                  options: (f.scorecard?.options || []).length
                    ? (f.scorecard?.options || []).map((o) => ({ _key: genKey(), label: o.label, score: String(o.score) }))
                    : [
    ...
    try {
      const payload = {
        ...
        fields: fields.map(f => ({
          ...
          scorecard: f.fieldType === 'SCORECARD' || f.fieldType === 'SCORED_CHOICE'
            ? {
                ...(f.fieldType === 'SCORECARD' && f.scorecard?.weight?.trim()
                  ? { weight: Number(f.scorecard.weight) }
                  : {}),
                options: (f.scorecard?.options || [])
                  .map((o) => ({ label: o.label.trim(), score: Number(o.score) }))
                  .filter((o) => o.label.length > 0),
              }
            : undefined,
